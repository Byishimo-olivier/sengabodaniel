...
const Sidebar = ({ isOpen = false }) => {
    ...
    const NavLink = ({ to, children, icon, onClick, badge }) => (
        <Link
            to={to}
            className="flex items-center justify-between px-4 py-2 rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-800 transition-all duration-200 group"
            onClick={() => {
                onClick?.();
            }}
        >
    ...
    const SectionHeader = ({ children, icon, expanded, onToggle }) => (
        <button
            onClick={onToggle}
            className="flex items-center justify-between w-full px-3 py-1.5 text-sm font-semibold text-gray-400 hover:text-white transition-colors duration-200"
        >
    ...
    const SubNavLink = ({ to, children, onClick }) => (
        <Link
            to={to}
            className="flex items-center px-6 py-1.5 text-sm text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition-all duration-200 ml-3"
            onClick={() => {
                onClick?.();
            }}
        >
            <span className="w-1.5 h-1.5 bg-gray-600 rounded-full mr-2"></span> {/* Reduced size and margin */}
            {children}
        </Link>
    );
        ...
        <>
            ...
            <aside
                className={`fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0
                    w-56 bg-gradient-to-b from-gray-900 via-gray-900 to-black shadow-2xl
                    transition-transform duration-300 ease-in-out z-40 md:z-auto overflow-y-auto pt-12`}
            >
