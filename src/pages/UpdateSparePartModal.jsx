...
const UpdateSparePartModal = ({ part, onClose, onUpdateSuccess, showMessage }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${API_BASE_URL}/spare_parts/${part.PartID}`, {
        ...formData,
      ...
      onUpdateSuccess(); // Call success callback from parent
    } catch (error) {
      console.error('Error updating item:', error);
      if (error.response) {
        showMessage(error.response.data.message || 'Failed to update item.', 'error');
      } else {
